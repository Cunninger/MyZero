...
const ResultPage = () => {
  ...
  return (
    <div className="p-5 sm:p-6 lg:p-8 space-y-5 animate-fade-in">
      ...
      <div className="flex items-center gap-2">
        {(() => {
          const mc = getModeColor(record.mode)
          return (
            <span className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full font-medium border ${mc.bg} ${mc.text} ${mc.chipBorder}`}>
      ...
      <div className="space-y-4">
        {activeTab === 'result' && (record.status === 'completed' || getOptimizedText()) && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 animate-fade-in">
            {/* Optimized Text */}
            <div className="bg-white rounded-2xl border border-slate-200/80 overflow-hidden flex flex-col h-[520px] shadow-sm shadow-slate-200/50 ring-1 ring-slate-100">
              <div className="px-4 py-2.5 border-b border-slate-100/80 flex justify-between items-center bg-gradient-to-r from-white to-primary-50/30">
                <div className="flex items-center gap-2">
                  <div className="w-1.5 h-4 rounded-full bg-gradient-to-b from-primary-500 to-blue-500" />
                  <h4 className="text-[13px] font-semibold text-slate-700">优化后的文本</h4>
                </div>
                <div className="flex items-center gap-1">
                  <button
                    onClick={() => handleCopy(getOptimizedText())}
                    className="text-[11px] text-slate-500 hover:text-primary-600 px-2 py-1 hover:bg-primary-50 rounded-md transition-colors flex items-center gap-1"
                  >
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4">
                <pre className="whitespace-pre-wrap font-serif text-[14px] text-slate-800 leading-[1.8]">
                  {getOptimizedText()}
                </pre>
              </div>
            </div>
            ...
            {/* Original Text */}
            <div className="bg-white rounded-2xl border border-slate-200/80 overflow-hidden flex flex-col h-[520px] shadow-sm shadow-slate-200/50 ring-1 ring-slate-100">
              <div className="px-4 py-2.5 border-b border-slate-100/80 bg-gradient-to-r from-white to-slate-50/60">
                <div className="flex items-center gap-2">
                  <div className="w-1.5 h-4 rounded-full bg-gradient-to-b from-slate-300 to-slate-400" />
                  <h4 className="text-[13px] font-semibold text-slate-400">原始文本</h4>
                </div>
              </div>
              <div className="flex-1 overflow-y-auto px-5 py-4 bg-slate-50/30">
                <pre className="whitespace-pre-wrap font-serif text-[14px] text-slate-500 leading-[1.8]">
                  {getOriginalText()}
                </pre>
              </div>
            </div>
      ...
      {activeTab === 'result' && !getOptimizedText() && getOriginalText() && (
        <div className="bg-white rounded-2xl border border-slate-200/80 overflow-hidden flex flex-col h-[520px] shadow-sm shadow-slate-200/50 ring-1 ring-slate-100 animate-fade-in">
          <div className="px-4 py-2.5 border-b border-slate-100/80 bg-gradient-to-r from-white to-slate-50/60">
            <div className="flex items-center gap-2">
              <div className="w-1.5 h-4 rounded-full bg-gradient-to-b from-slate-300 to-slate-400" />
              <h4 className="text-[13px] font-semibold text-slate-400">原始文本</h4>
